...
- **Single-teacher-per-student assumption is intentional but bounded** — Current model assumes PreK–8 homeroom teachers where one teacher authors all sections of a student's profile. Middle/high school (subject teachers, classes, narrower auth scope, multi-author profiles) is a future build, not a bug. **Don't write code that hardcodes "one teacher per student" as a precondition.** Specifically: (a) route teacher access through helpers like `canTeacherAccessStudent(userId, studentId)` even when they're trivial today, so the future swap to class-scoped checks is a one-file diff; (b) don't bake "your profile" / single-author copy into Profile Builder workspace UI; (c) treat `season` as an open string in business logic, never `if (season === 'fall')` branches (quarters/trimesters come later); (d) don't add compound unique constraints that block future `class_id` / `school_class_id` columns on `profile_sections`. Three open questions to answer when the first middle/high customer lands: does the profile belong to the student or the class, who has submit power per section vs. per profile, and do parents see per-author attribution.
