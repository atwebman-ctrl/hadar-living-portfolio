...
- **Per-school subject configuration UI (NEAR-FUTURE)** — Scripture/Bible/Torah is read at nearly every target school; Aaron estimates only ~3 in the addressable market skip it formally, and even Hadar reads Bible/Torah under different framing. So the current state (Scripture as a required section, placeholder card, shipped 2026-06-02) is closer to correct than wrong — it's the common case, not an edge case. The follow-up is a settings UI at `/dashboard/settings` where admins check boxes for which subjects/sections appear in their tenant's reports. Likely shape: extend dormant `schools.enabled_sections` or add `schools.profile_section_kinds_enabled` array. Design the whole subject taxonomy as configurable (Scripture/Torah, Hebrew/Latin/Greek, art_projects, etc.) — not a one-off Scripture toggle. Support per-tenant **label override** too (Hadar might rename "Scripture" to "Torah" rather than hide it). Remaining Scripture-specific code work after the UI: `lib/sectionData.ts` loader, dedicated editor wrapper, AI prompt builder in `lib/aiPrompts.ts`, dispatch case in `app/api/.../generate/route.ts` — those are real-content wiring rather than placeholder shipping.
- **Latin/Greek as third languages for classical Christian tenants** — current `third_languages` config supports Hebrew (Hadar). Classical Christian schools will need Latin and Greek slots instead. Schema already accommodates this (it's just JSONB on `schools.third_languages`), but per-tenant seeding + any AVANT-norm equivalents need a follow-up pass. Folds into the per-school subject configuration UI above.
