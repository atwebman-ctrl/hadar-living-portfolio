...
- **Hero section: identity only** — Photo + name only. No school name badge in the hero — it would duplicate the sidebar. School identity lives in the hub branding block below the group cards (`PortfolioHub.tsx`). Metrics live in `StatsBar` below the hero, not inside it.
