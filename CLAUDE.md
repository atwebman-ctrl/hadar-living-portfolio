...
- **Scripture tab — per-tenant gating + Profile Builder wiring** — Scripture (added 2026-06-02 for classical Christian school audiences) currently ships globally with a "Coming soon" empty state in the live portfolio tab strip. Two follow-ups: (a) per-tenant `enabled_sections` config on `schools` so Hadar can hide it and Christian-school tenants can show it without code changes, mirroring the `third_languages` JSONB pattern; (b) full Profile Builder section — new `'scripture'` value in `PROFILE_SECTION_KINDS` (DB CHECK constraint migration required), `lib/sectionData.ts` loader, editor wrapper, AI prompt builder in `lib/aiPrompts.ts`, dispatch case in `app/api/.../generate/route.ts`, and a `PublishedSectionRenderer` block. The live tab and the published Profile Builder report are separate parent-facing surfaces; only the live tab is wired today.
- **Latin/Greek as third languages for classical Christian tenants** — current `third_languages` config supports Hebrew (Hadar). Classical Christian schools will need Latin and Greek slots instead. Schema already accommodates this (it's just JSONB on `schools.third_languages`), but per-tenant seeding + any AVANT-norm equivalents need a follow-up pass.
